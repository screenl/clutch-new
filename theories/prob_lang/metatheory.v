...
From clutch.prelude Require Import stdpp_ext NNRbar fin uniform_list tactics.
...
Proof.
  revert X. induction e=> X /=;
   ...
   repeat case_decide; simplify_eq_d => /=; f_equal; intuition eauto with set_solver.
Qed.
...
Proof.
  ...
  - intros m i x Hi Hix Hkwm Hm.
    ...
    + simplify_eq_d => /=. rewrite !Z2Nat.id in Hlt; eauto with lia.
...
Proof.
  ...
  - rewrite /lmarg dmap_comp /state_step.
    ...
    + rewrite {1 2}dret_1_1 // dret_0; [lra|].
      intros [= H%(insert_inv (tapes σ))]. simplify_eq_d.
...
Proof.
  ...
  - pose (witness:=dmap (λ n: fin 4, ( match fin_to_nat n with
    ...
    + intros [??].
      ...
      intros [?[??]].
      repeat (inv_fin x => x); simpl in *; simplify_eq_d => _; naive_solver.
      ...
      -- rewrite /lmarg dmap_comp.
         ...
         ** apply distr_ext. intros s.
            ...
            { repeat rewrite bool_decide_eq_false_2.
              - lra.
              - subst. intro K. simplify_eq_d. rewrite map_eq_iff in K.
                specialize (K α2). rewrite !lookup_insert_eq in K. simplify_eq_d.
              - subst. intro K. simplify_eq_d. rewrite map_eq_iff in K.
                specialize (K α1). rewrite lookup_insert_ne in K; last done.
                rewrite (lookup_insert_ne (<[_:=_]> _ )) in K; last done.
                rewrite !lookup_insert_eq in K. simplify_eq_d.
              - subst. intro K. simplify_eq_d. rewrite map_eq_iff in K.
                specialize (K α2). rewrite !lookup_insert_eq in K. simplify_eq_d.
            }
            case_bool_decide.
...
Proof.
  ...
  - (* σ' is reachable *)
    ...
    { intros a Ha.
      rewrite bool_decide_eq_false_2; first lra.
      move => /state_upd_tapes_same. intros L. simplify_eq_d.
    }
    ...
    + rewrite SeriesC_singleton_dependent. erewrite state_step_unfold; last first.
      ...
      { intros a H'. rewrite bool_decide_eq_false_2; first lra.
        ...
        simplify_eq_d. apply H'.
        rewrite -K in Hsome.
        rewrite last_snoc in Hsome. by simplify_eq_d.
      }
      rewrite SeriesC_singleton_dependent.
    ...
    + (* prove that σ' is not an intermediate step*)
      ...
      apply state_upd_tapes_same in H2. rewrite -app_assoc in H2. simplify_eq_d.
  ...
  - split; apply distr_ext.
    + intros v. rewrite lmarg_pmf.
      ...
      * rewrite SeriesC_singleton_dependent. rewrite dmap_unfold_pmf.
        ...
        intros.
        case_bool_decide; simplify_eq_d.
    ...
    + destruct H as [n ->].
      ...
      * symmetry.
        ...
        { destruct e as [m <-] eqn:He.
          ...
          - apply dret_0.
            move => /state_upd_tapes_same. intros eq. simplify_eq_d.
...
Proof.
  intros H1 H2.
  inversion H1; inversion H2; simplify_eq_d; auto.
Qed.
...
Proof.
  split.
  - intro H; inversion H; simplify_eq_d; eexists; eexists; econstructor; eauto.
...
Local Ltac inv_det_head_step :=
  repeat
    ...
    | H : is_det_head_step _ _ = true |- _ =>
        ...
        repeat (case_match in H; simplify_eq_d)
...
Proof.
  intros Hdet.
  apply pmf_1_eq_dret.
  inversion Hdet; simplify_eq_d => /=; try case_match;
...
Proof.
  intros [] [Hs | Hs]; inversion Hs; simplify_eq_d.
Qed.
...
Proof.
  split.
  - intros [Hdet | Hdet];
      inversion Hdet; simplify_eq_d; do 2 eexists; try (by econstructor).
    Unshelve. 4: apply 0%Z. all: apply 0%fin.
  - intros (?&?& H). inversion H; simplify_eq_d;
