...
Section adequacy.
  ...
  Proof.
    ...
    - iApply step_fupdN_mono.
      ...
      iApply (step_fupdN_mono _ _ _ ⌜(pgl _ φ (ε1' + ε2')) ⌝).
  ...
  Proof.
    ...
    - iDestruct (big_orL_mono _ (λ _ _,
                     |={∅}▷=>^(S n)
                       ⌜1 - SeriesC (iterM (S n) prim_step_or_val (e1, σ1)) + prob (exec (S n) (e1, σ1)) (λ v, Datatypes.negb (bool_decide (φ v))) <= ε''⌝)%I
                  with "H") as "H".
      { iIntros (i α Hα%list_elem_of_lookup_2) "(% & %ε1 & %ε2 & (%r & %Hε'') & %Hleq & %Hlift & H)".  
...
Proof.
  ...
  iMod (ec_alloc ε') as (?) "[Hera Her]"; [done|].
  set (HclutchGS := HeapG Σ _ _ _ γH γT _).
  iPoseProof (wp_refRcoupl_step_fupdN ε' e σ n φ with "[Hh Ht Hera Her]") as "H";
  iFrame.
  iApply Hwp.
  done.
Unshelve.
...
Proof.
  ...
  iMod (ec_alloc ε') as (?) "[Hera Her]"; [done|].
  set (HclutchGS := HeapG Σ _ _ _ γH γT _).
  iPoseProof (wp_safety_fupdN ε' e σ n φ with "[Hh Ht Hera Her]") as "H";
  iFrame.
  iApply Hwp.
  done.
Unshelve.
...
Proof.
  ...
  iMod (ec_alloc ε') as (?) "[Hera Her]"; [done|].
  set (HclutchGS := HeapG Σ _ _ _ γH γT _).
  iPoseProof (wp_adequate_step_fupdN ε' e σ n φ with "[Hh Ht Hera Her]") as "H";
  iFrame.
  iApply Hwp.
  done.
  Unshelve.
